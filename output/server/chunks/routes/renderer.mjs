...
import {
  ...
  v as getResponseStatusText,
  x as getResponseStatus,
  y as defineRenderHandler,
  z as publicAssetsURL,
  A as getQuery,
  a as createError,
  B as destr,
  C as getRouteRules,
...
const getPrecomputedDependencies = () =>
...
const getSPARenderer = lazyCachedFunction(async () => {
  const precomputed = await getPrecomputedDependencies();
  const spaTemplate = await import("../virtual/_virtual_spa-template.mjs")
    .then((r) => r.template)
    .catch(() => "")
    .then((r) => {
      {
...
const getSSRStyles = lazyCachedFunction(() =>
  import("../build/styles.mjs").then((r) => r.default || r),
);
...
const APP_TELEPORT_OPEN_TAG = HAS_APP_TELEPORTS
  ? `<${appTeleportTag}${propsToString(appTeleportAttrs)}>`
  : "";
...
const renderer = defineRenderHandler(async (event) => {
  ...
  const inlinedStyles =
    !ssrContext._renderResponse && !isRenderingPayload
  ...
  const { headTags, bodyTags, bodyTagsOpen, htmlAttrs, bodyAttrs } = await renderSSRHead(
    ssrContext.head,
