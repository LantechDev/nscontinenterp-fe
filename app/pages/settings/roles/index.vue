...
import { Plus, Search, Shield, Edit, Trash2, Loader2, AlertTriangle, ArrowLeft } from "lucide-vue-next";
...
<template>
    <div class="space-y-6 animate-fade-in">
        ...
        <div class="card-elevated p-4">
            <div class="relative">
                ...
                <input v-model="searchQuery" type="text" placeholder="Cari role (nama, code)..."
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr v-else-if="filteredRoles.length === 0">
                        <td colspan="5" class="text-center p-8 text-muted-foreground">Tidak ada role ditemukan.</td>
                    </tr>
                    <tr v-else v-for="role in filteredRoles" :key="role.id"
                        ...
                        <td>
                            <span
                                :class="['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium', role.isActive ? 'badge-success' : 'bg-muted text-muted-foreground']">
                                {{ role.isActive ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td>
                            <div class="flex items-center gap-2">
                                <NuxtLink :to="`/settings/roles/${role.id}/edit`"
    ...
    <div v-if="showDeleteConfirm"
        ...
        <div class="bg-background rounded-lg shadow-lg max-w-md w-full p-6 mx-4">
            ...
            <p v-if="errorRoot" class="bg-red-50 text-red-600 p-2 rounded text-sm mb-4 border border-red-200">
                {{ errorRoot }}
            </p>

            <p class="text-muted-foreground mb-6">
                Apakah Anda yakin ingin menghapus role <strong>{{ roleToDelete?.name }}</strong>? Tindakan ini tidak
                dapat dibatalkan.
            </p>
            <div class="flex justify-end gap-3">
                <button type="button" @click="cancelDelete" class="btn-secondary" :disabled="isDeleting">
                    Batal
                </button>
