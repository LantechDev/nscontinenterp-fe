...
// Auto-generate slug from name if not editing and slug is empty
watch(
    () => form.value.name,
    (newName) => {
...
const initData = async () => {
    // We need to find the active organization object
    // For simplicity, let's assume we can get it from listOrganizations for now if not in session
    // Or maybe we should improve useAuth to expose `activeOrganization` computed.
    // Let's fetch list.
    // Actually, looking at session structure, we might not have full metadata.
    // Let's fetch the list and find the active one.
    // NOTE: In a real app we might want a specific `getOrganization` endpoint, but `list` is cache-friendly and usually small collection.

    // Check if we already have it populated in form (avoid reset on typing)
    if (form.value.name) return;

    isLoading.value = true;
    try {
        // useAuth wrapper usually handles loading state, but we want local loading control
        // Accessing the listOrganizations from the composable
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (result.success) {
            ...
            errorRoot.value =
                result.error ||
                (isEditing.value ? "Gagal mengupdate tenant." : "Gagal membuat tenant.");
        }
    } catch (e: any) {
        console.error("TenantSettings: Error", e);
