...
const { quotations: quotationsList, isLoading, fetchQuotations } = useQuotations();
...
// Transform API quotations to view format
const quotations = computed(() => {
  return quotationsList.value.map((q: Quotation) => ({
    id: q.id,
    number: q.number,
    customer: q.customer?.name || "-",
    date: formatDate(q.date),
    amount: formatAmount(q.total),
    status: q.status.toLowerCase() as "sent" | "draft" | "accepted" | "rejected",
  }));
});
